...
**Where this fits:** the finish line. Sections 1–19 each added one capability; this
...
> auto tool choice) and, for retrieval, an **embedding model** (`EMBED_MODEL`, Section 15)
...
```
question ─▶ agent loop ─▶ [ search_kb tool ]  (retrieval, §15/16)
                         [ calculate tool ]   (tools, §13/14)
                              │
                         every model call is logged (§9) and costed (§10)
                              │
                         final answer (grounded; "I don't know" if unknown, §16/17)

then: a tiny eval suite scores it (§19)
```
...
**2. Retrieval tool (§15, §16)** — search the knowledge base. Use embeddings if
...
**3. A safe tool + registry (§13, §17)** — the calculator, with the no-`eval` parser, and
...
**4. The agent loop (§14, §18)** — the same engine, now using `chat()` so every step is
...
[`examples/20/capstone.py`](../examples/20/capstone.py).)*
...
1. **Real RAG with chunking.** Replace `DOCS` with a real document split into chunks
   (Section 16), and set `EMBED_MODEL` so retrieval is semantic.
...
3. **A guardrail with teeth.** Add a `create_ticket(summary)` tool that requires
   confirmation before "acting" (Section 17), and validate its arguments with Pydantic
   (Section 6).
...
5. **Grow the eval.** Turn the two cases into a real golden suite plus an LLM-as-judge for
   answer quality (Section 19), and run it before/after each change.

---

## You made it — what you can now do

Across twenty sections you built, from primitives, the ability to:
...
- **Build real applications** — prompt engineering, conversation memory, tools, the tool
  loop, embeddings, RAG, security, agents, and evaluation (§11–19).
...
> [`examples/20/capstone.py`](../examples/20/capstone.py). Read it once you've built your
