...
linkTitle: '10. Observability & Logging'
weight: 10
---
...
**Where this fits:** Section 2 introduced `usage` and `finish_reason`; Section 9 added
errors and retries. This lesson collects all of it into one record per call — the data
you'll need to compute cost in Section 11 and to understand what your app is doing.
...
[`examples/10/log_calls.py`](../examples/10/log_calls.py).)*
...
- **`session_id`** — the whole conversation / user session. Stable across many turns (the
  message history from Section 13 is one session).
...
- **Every emit site uses the same shape.** A model call and a tool result that carry the
  *same* `trace_id`/`step` join up; if only model calls are stamped, the tool half of the
  story is invisible. (Section 23 makes both halves emit the tuple.)
...
- **Track cost** — sum `prompt_tokens` and `completion_tokens` over time (Section 11 turns
  these into dollars).
...
- Stamp every record with a **`session_id` / `trace_id` / `step`** so related calls
  *join* — the foundation for debugging multi-step agents (Section 23). Server ids
