...
keywords — which is the foundation for retrieval (Section 20) and a core building block
...
[`examples/19/embed.py`](../examples/19/embed.py).)*
...
[`examples/19/similarity.py`](../examples/19/similarity.py).)*
...
- **Semantic search / retrieval** — find the most relevant chunks of text (Section 20).
...
> We'll stay with the brute-force numpy version in Section 20 so the mechanics stay
> visible.
