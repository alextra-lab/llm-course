...
> [`examples/05/reasoning.py`](../examples/05/reasoning.py).)*
...
- It spends the **output side of the context budget** (Section 3).
- It is **billed like output** (Section 10).
...
> [`examples/05/reasoning_effort.py`](../examples/05/reasoning_effort.py).)*

---

## Don't feed the thinking back in

A rule for multi-turn conversations (Section 12): when you continue a conversation, send
...
- Reasoning is generated text: it's in `completion_tokens`, spends the **context budget**
  (Section 3), and is **billed** (Section 10).
...
**Section 6 — Handling & Validating Responses:** text you can read is not data you can
