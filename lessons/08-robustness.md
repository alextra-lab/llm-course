...
something you'd run unattended — and it sets up clean logging in Section 9.
...
*(Reference: [`examples/08/handling_errors.py`](../examples/08/handling_errors.py).)*
...
> [`examples/08/retries.py`](../examples/08/retries.py).)*
...
- **Timeouts.** Always set one (`timeout=` on the client or per call). Without it, a stuck
  connection hangs forever. Reasoning requests (Section 5) can legitimately take a while —
...
**Section 9 — Observability & Logging:** now that calls can fail and recover, you'll make
