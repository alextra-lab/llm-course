...
here we contain *what code can touch*. Everything an agent (Section 22) runs unattended
...
> **Opt-in:** the runnable [`examples/16/pg_sandbox.py`](../examples/16/pg_sandbox.py) only
...
[`examples/16/audit_log.py`](../examples/16/audit_log.py).
...
3. **Wire the audit log.** Make `run_command` (Section 15) and `run_in_container` both emit
...
- **Audit** every execution as metadata-only JSON; ship it to the Elastic stack (Section 9).

## Next

**Section 17 — Model Context Protocol (MCP):** with tools defined (13–14) and their execution
