...
linkTitle: '7. Handling & Validating Responses (Structured Output)'
weight: 7
---
...
for tool calling (Section 14) and agents (Section 23).
...
but not enough. *(Reference: [`examples/07/json_mode.py`](../examples/07/json_mode.py).)*
...
> typed objects from one definition. We'll reuse this to define tools in Section 14.
> *(Reference: [`examples/07/json_schema_pydantic.py`](../examples/07/json_schema_pydantic.py).)*
...
`max_tokens` and treat a parse failure as a real error path (Section 9), not a surprise.
