...
linkTitle: '12. Prompt Engineering Fundamentals'
weight: 12
---
...
"chain of thought" relates to our model's *native* reasoning (Section 6).

**Where this fits:** this opens the advanced arc. You've controlled the model with
*parameters* (Sections 5–6); now you control it with *words*. Everything here reuses the
...
[`examples/12/zero_vs_few_shot.py`](../examples/12/zero_vs_few_shot.py).)*

> **Why `temperature=0` here?** You're comparing prompts, so you want the *prompt* to be
> the only thing that changes. Pinning temperature (Section 5) keeps the comparison fair.
...
2. **Delimit the data.** Wrap user-supplied text in clear markers (`<doc>…</doc>`,
   ...
   *data*. This also matters for safety — Section 21.
3. **Show the shape you want.** Ask for JSON (Section 7), a table, or bullets — and say
   so explicitly.
...
[`examples/12/structure.py`](../examples/12/structure.py).)*
...
steps. But `gpt-oss-120b` is a **reasoning model** (Section 6) — it *already* thinks
...
for *you* to validate (Section 24).

---

> **Security:** The highest-value habit in the whole course: keep **instructions and data apart** — instructions in the system message, untrusted text **delimited**. It's exactly what defends against prompt injection in Section 21.
...
2. **Format lock-in.** Rewrite `work/structure.py` to output a strict JSON object
   (combine with Section 7's `response_format`). *Success:* the output parses with
   `json.loads`.
