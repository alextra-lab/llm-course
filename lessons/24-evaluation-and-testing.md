...
linkTitle: '24. Evaluation & Testing'
weight: 24
---
...
**Where this fits:** Sections 12–23 made the model *do* things. This section makes those
...
[`examples/24/golden_test.py`](../examples/24/golden_test.py).)*
...
(Section 7) so a score is always a number in range. Create **`work/judge.py`**:
...
*(Reference: [`examples/24/eval_judge.py`](../examples/24/eval_judge.py).)*
...
- **Track quality *and* cost.** A change that improves scores but doubles tokens
  (Section 11) may not be worth it. Log both (Section 10).
...
3. **Eval an earlier section.** Point a golden test at your Section 20 RAG: questions your
...
- **LLM-as-judge** grades open-ended output against a rubric; validate the judge's score
  (Section 7) and beware judge bias.
