...
In Section 13 you wrote a tool schema, sent it to the model, and ran the matching Python
...
tool schema you already wrote in Section 13:
...
**`work/mcp_server.py`**. It exposes two tools: the Section 13/14 `calculate`, and a small
...
> implementation. *(Reference: [`examples/17/mcp_server.py`](../examples/17/mcp_server.py).)*
...
[`examples/17/mcp_client.py`](../examples/17/mcp_client.py).)*

---

## Bridge MCP tools into the tool-use loop

The payoff: let the *model* use the server's tools. The Section 14 loop already does the
...
advertisement into the OpenAI schema from Section 13:
...
The loop is Section 14, with the tool list discovered over MCP and each call dispatched to
...
exactly the pattern from Section 16:
...
the reuse MCP buys you. *(Reference: [`examples/17/mcp_bridge.py`](../examples/17/mcp_bridge.py).)*
...
tools and another owns the app. For a single script with two local functions (Section 14),
...
3. **Sandbox a dangerous tool.** Add a `run_python(code)` tool to the server that runs the
   code behind the Section 15 sandbox (`run_untrusted`). Call it with an infinite loop.
...
- A server is a trust boundary: scope credentials, reject tools it didn't advertise,
  validate I/O (Section 20), and sandbox anything it runs (Sections 15–16).
