...
**Where this fits:** every call in Sections 1–6 was blocking — the simple default.
...
*(Reference: [`examples/07/raw_sse.py`](../examples/07/raw_sse.py).)*
...
> `choices` is empty) — you need this for cost accounting (Section 10) on streams.
...
> *(Reference: [`examples/07/sdk_stream.py`](../examples/07/sdk_stream.py).)*
...
**structured output** (Section 6): you can't validate JSON against a schema until you've
...
3. **Stream then validate.** Combine with Section 6: stream a JSON answer, reassemble the
...
**Section 8 — Robustness:** real networks and servers fail. You'll handle errors, rate
