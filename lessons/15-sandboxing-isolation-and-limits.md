...
**Where this fits:** this is the missing half of tool use. Section 14 gave the model the
power to *act*; this section makes acting *contained*, so that by the time you reach
Agents (Section 22) — which take many steps unattended — every step runs inside a limit.
...
1. **Don't execute at all** — parse/validate instead (the Section 13–14 calculator). Best
   when you can.
...
4. **microVMs / user-space kernels** *(Section 16, pointers)* — gVisor, Firecracker; what
...
> (Section 16) close. Know what your tier does *not* protect.
...
Section 16 (which removes the files and network entirely).

> **Reference:** the complete, runnable versions are
> [`examples/15/safe_exec.py`](../examples/15/safe_exec.py) and
> [`examples/15/bash_allowlist.py`](../examples/15/bash_allowlist.py). Run them and watch
...
**Section 16 — Sandboxing II:** we climb the ladder. Containers add the filesystem and
