...
(Section 2), reasoning tokens (Section 5), and your logs (Section 9).
...
- **Reasoning tokens are output tokens.** `gpt-oss-120b`'s thinking (Section 5) is part of
...
Combine this with Section 9's logging and you can sum cost per request, per user, per day
— exactly. *(Reference: [`examples/10/cost.py`](../examples/10/cost.py).)*
...
*(Reference: [`examples/10/prompt_cache.py`](../examples/10/prompt_cache.py).)*
...
> `usage`. Monitor it (Section 9) where you can.
...
2. **Cost the reasoning tax.** Run a hard prompt at `reasoning_effort="high"` vs `"low"`
   (Section 5) and cost each. *Success:* you can put a dollar figure on the extra thinking.
...
- Compute cost straight from `response.usage`; combine with Section 9 logs to track spend.
