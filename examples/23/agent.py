"""
Section 23 - An agent: the tool loop + a goal + planning + multiple tools.

This is Section 15's loop, given a system prompt that tells the model to plan and
...
habits from Section 21: a tool registry (only known tools run), validated/typed
arguments, tool errors returned to the model, and a hard step cap.

It also carries Section 10's joinable telemetry into the agent: every model call
...
# A small knowledge base. (A real agent would search this with RAG -- Section 20.
