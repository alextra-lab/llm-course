"""
Section 17 - A tiny MCP server over stdio (JSON-RPC 2.0).
...
Section 13/14 `calculate` and a small `doc_search` -- and runs them on request.
...
mcp_client.py). Run it directly with empty stdin and it exits cleanly at EOF.
    ...
    python examples/17/mcp_client.py              # the normal way to drive it
    printf '' | python examples/17/mcp_server.py  # clean EOF exit
"""
...
# --- the tools this server exposes (reused from Sections 13-14) ----------------
