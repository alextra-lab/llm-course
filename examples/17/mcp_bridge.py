"""
Section 17 - Bridge MCP tools into the Section 14 tool-use loop.

The server advertises tools; here we translate that list into the OpenAI tool
schema the model expects, run the Section 14 loop, and -- when the model asks for
...
the only part that needs the endpoint, so without OPENAI_* set this skips cleanly.
    ...
    python examples/17/mcp_bridge.py
"""
...
def to_openai_schema(mcp_tool):
    """An MCP tool advertisement -> the OpenAI function-tool schema (Section 13)."""
