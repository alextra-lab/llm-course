"""
Section 16 - Sandboxing I: run untrusted code in a limited subprocess.

The tool loop (Section 15) lets the model choose what to run. For a calculator we
...
NOT fully block the filesystem or the network -- that needs a container (Section 17).

    python examples/16/safe_exec.py
"""
...
if __name__ == "__main__":
    ...
    print("Each ran in its own process; the dangerous ones were stopped by limits,\n"
          ...
          "limit; macOS ignores some) -- which is exactly why Section 17 reaches for\n"
