...
([Foundations Lesson 13 — Conversation State & Memory](../../lessons/13-conversation-state-and-memory.md)), and eventually that list will not fit in the model's context window. That is the
...
the foundations course: [Foundations Lesson 4 — Tokens & the Context Window](../../lessons/04-tokens-and-the-context-window.md), [Foundations Lesson 11 — Cost, Pricing & Prompt Caching](../../lessons/11-cost-pricing-and-prompt-caching.md), [Lesson 13 — Conversation State & Memory](../../lessons/13-conversation-state-and-memory.md), and [Lesson 23 — Agents](../../lessons/23-agents.md). The Agent Memory course began by setting
...
every turn. An agent ([Lesson 23 — Agents](../../lessons/23-agents.md)) does the same, and adds tool calls and their results to the list
...
- the **tool definitions** — schemas for every tool the agent can call ([Lesson 23 — Agents](../../lessons/23-agents.md));
- **retrieved context** — memory or RAG passages injected for this turn ([Foundations Lesson 20 — Retrieval-Augmented Generation (RAG)](../../lessons/20-retrieval-augmented-generation.md));
- the **conversation history** — every prior user, assistant, and tool message ([Lesson 13 — Conversation State & Memory](../../lessons/13-conversation-state-and-memory.md));
...
```python
# A long session is a messages list that keeps growing (Foundations Lesson 13).
...
runtime-memory trick that frees no space in your window) and from *prompt caching* ([Lesson 11 — Cost, Pricing & Prompt Caching](../../lessons/11-cost-pricing-and-prompt-caching.md), which
...
2. **Approaching the budget?** Drop or window the oldest turns first ([Lesson 13 — Conversation State & Memory](../../lessons/13-conversation-state-and-memory.md)) — cheap, and
...
> its output changed — using the joinable `session_id`/`trace_id`/`step` line from
> [Foundations Lesson 10 — Observability & Logging](../../lessons/10-observability-and-logging.md). A compression you cannot see is one you cannot trust, so you start measuring in Unit 1,
...
1. **Feel the budget run out.** Take any [Lesson 13 — Conversation State & Memory](../../lessons/13-conversation-state-and-memory.md) or [Lesson 23 — Agents](../../lessons/23-agents.md) script, keep appending turns (or let an
...
**[Unit 1 — Measuring the Window](01-measuring-the-window.md):** before you can manage a budget you have to read it. You
