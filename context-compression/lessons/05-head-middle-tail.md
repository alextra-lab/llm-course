...
**fraction of the budget** in tokens, and it spans at least a **minimum number of turns.**

```python
def _tail_start(messages, head_end, budget, ratio=0.25, min_turns=4):
    floor = ratio * budget
    start = len(messages)
...
summary of its own last move. But a token floor alone fails when recent turns are tiny — four
short exchanges might be 200 tokens, well under the floor, so a token-only rule would walk back and
swallow half the conversation to reach its quota. The **turn floor** (about **4 turns**) guarantees
a minimum number of recent exchanges survive verbatim regardless of their size. Whichever floor
binds last is the one that sets the boundary. In the example, the token floor is met three turns
back, but the turn floor pulls in one more — you can watch both rules act.
...
invariant: head verbatim True | tail verbatim True | tail ever compressed False
```

That "byte-for-byte" is not only correctness — it is a cache property (§10). The head never
changes, so its cached prefix is reused every turn; the tail only ever grows by appending; and the
single region that *does* change is the recap in the middle. That is the best you can do for the
cache while still compressing: you confine the damage to one place. Unit 9 takes the last step —
freeze even that region between scheduled rebuilds — but the head/tail split is what makes a frozen
layout possible at all.

## The honest part

The numbers are dials, not laws. `0.25` of the budget and `4` turns are one harness's choices;
a workload with long, dense recent turns wants a smaller token fraction, and a chatty one with
tiny turns wants a larger turn floor. There is a real tradeoff hiding in the tail floor: every
token you reserve for a verbatim tail is a token you have chosen *not* to compress, so a generous
tail leaves less headroom and trips the threshold sooner. Set it from the workload, and measure.
...
the middle is not simply dropped: it is summarized (Unit 4), and when the middle's bulk is a giant
tool output, pre-collapsed cheaply first (Unit 6) or offloaded whole (Unit 8) rather than handed to
a summarizer at all.
...
> `head_tokens`/`middle_tokens`/`tail_tokens`, and a `tail_compressed` flag — using the §9 joining
> tuple. The loop it closes is the invariant itself: `tail_compressed` must be `false` on every
> record, so the log *proves* the mechanism never touched a turn the model was using, rather than
> asserting it. The three token counts also show, run over run, how the split is moving — a middle
> that keeps growing while the tail floor holds steady is the signal that compression is doing its
> job on the right region.

## Challenges

1. **Find the binding floor.** Run the example and read the split line. *Success:* you can say
   which floor set the tail boundary (token or turn), and predict how the split changes if you
   halve `ratio` or raise `min_turns` — then change them and confirm.
2. **Prove the invariant.** Confirm the printed check shows head and tail verbatim and
   `tail ever compressed False`. *Success:* you can explain why the check uses object identity
   (`is`) rather than equality, and what bug an equality check would hide.
...
- The model attends most to the **head and the tail** (Unit 0's U-shape read as a design rule), so
  protect both from compression — Unit 3's head-only anchor eventually lets the window eat the live
  tail.
- The **head** is leading system messages + the first user message. The **tail** is defined by
  **two floors**: at least a fraction of the budget in tokens (~0.25) *and* at least a minimum
  number of turns (~4) — whichever binds last sets the boundary.
...
- Be honest: the floors are **dials** with a real tradeoff (a bigger verbatim tail = less
  headroom), and the middle is the *safest* place to lose fidelity, **not a free one** — which is
  why the middle is summarized (Unit 4), pre-collapsed (Unit 6), or offloaded (Unit 8), never just
  deleted.

## Next

**Unit 6 — Cheap Before Smart: The Deterministic Pre-Pass:** the middle is usually mostly one
