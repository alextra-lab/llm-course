...
|---|---|---|---|
| **Working** | The current context — what's in the prompt *right now* | The active `messages` list | Ends with the session ([Foundations Lesson 13 — Conversation State & Memory](../../lessons/13-conversation-state-and-memory.md)) |
...
You already built **working memory** — it is the context management of [Lesson 13](../../lessons/13-conversation-state-and-memory.md), and it is *not*
...
```mermaid
flowchart TD
    subgraph Working["Working memory — ends with the session (Foundations 13)"]
...
- A **stateless Q&A bot** over documents needs **semantic** memory (the documents) and
  nothing else — that is plain RAG ([Foundations Lessons 19–20](../../lessons/19-embeddings.md)). Stop there; do not build episodic stores or a
  graph.
...
> `session_id`/`trace_id`/`step` line ([Foundations Lesson 10 — Observability & Logging](../../lessons/10-observability-and-logging.md)) should carry that kind, so you can later
...
- "Memory" is several kinds: **working** (context, [Lesson 13](../../lessons/13-conversation-state-and-memory.md)), **episodic** (events), **semantic**
...
**[Unit 2 — The Naive Baseline](02-the-naive-baseline.md):** enough theory — we build. You will persist turns to SQLite
