...
The foundations course ended Section 12 with a hint: *"Persisting facts across sessions is a
...
build on §12 (history), §18 (embeddings), §19 (RAG), and §22 (agents), and will not teach
them again.
...
You can see the gap with the foundations code. Run a §12 chat, then start the script again
...
> Unit 11):*
>
> 1. **Do you need memory across sessions at all?** No → window/summarize (§12). Stop. You're done.
> 2. **Are the facts mostly independent lookups?** Yes → a vector store / plain RAG
>    (§18–19) is enough. Do not build a graph.
...
- "Memory" names two unrelated problems: **context management** (keep one conversation
  consistent — §12) and **memory** (know things across sessions — this course).
