...
Every operation above also emits one **joinable telemetry line** on stderr — the
[Foundations Lesson 10 — Observability & Logging](../../lessons/10-observability-and-logging.md) shape (`session_id` / `trace_id` / `step`), with PII redacted at the boundary (Unit 10).
...
```mermaid
flowchart TD
    A{"Need memory<br/>across sessions?"} -->|No| B["Window / summarize (Foundations 13).<br/><b>Stop — you're done.</b>"]
    A -->|Yes| C{"Mostly independent<br/>fact lookups?"}
    C -->|Yes| E["Vector store / plain RAG (Foundations 19–20).<br/><b>Don't build a graph.</b>"]
...
- **A window or summary is enough.** If the agent only needs the last few turns, you do not need
  cross-session memory at all ([Foundations Lesson 13 — Conversation State & Memory](../../lessons/13-conversation-state-and-memory.md)). This is most chat features.
...
real agent from [Foundations Lesson 23 — Agents](../../lessons/23-agents.md). Whatever you build, keep the last rule of the tree: **measure
before you optimize.**
